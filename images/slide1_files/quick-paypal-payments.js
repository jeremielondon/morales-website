...
function validateForm(ev) {
	...
	action.val('qpp_validate_form');

	fd += '&' + c.attr('name') + '=' + c.val() + '&' + action.serialize();
	
	var fp = f.closest('.qpp-style');
		...
		if (e.responseJSON) {
			...
			if (data.errors.length) { // errors found
				...
				*/
				...
				// Invoke submit
				// f.find('input[type=submit]').last().click();
	...
	*/
	function updateTotal() {
		...
		if (f.processing_type) {
			...
			if (f.processing_type.qpp_value() == 'percent') PROCESSING = (.01 * PROCESSING) * SUB;
		}
		...
		if (f.postage_type) {
			...
			if (f.postage_type.qpp_value() == 'percent') POSTAGE = (.01 * POSTAGE) * SUB;
		}
